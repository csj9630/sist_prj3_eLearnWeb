...
public class UserReviewService {
	 ...
	 * @return
	 */
	public int totalCnt(UserReviewRangeDTO rangeDTO) {
		int totalCnt = 0;

		try {
			totalCnt = rm.selectReviewTotalCnt(rangeDTO);
	 ...
	 * @return
	 */
	public List<UserReviewDomain> searchReviewList(UserReviewRangeDTO rangeDTO) {
		List<UserReviewDomain> list = null;

		try {
			list = rm.selectRangeReview(rangeDTO);
			contentSubStr(list);
			decryptReviewAuthorInfo(list);
		} catch (SQLException e) {
			e.printStackTrace();
		} // end try ~ catch
	 ...
	 * @return
	 */
	public String pagination(UserReviewRangeDTO rangeDTO) {
		...
		/*
		 * prevMark.
		 * append("<li class='page-item'><a class='page-link' href='#'>Previous</a></li>"
		 * );
		 */
	...
	public String pagination2(UserReviewRangeDTO rangeDTO) {
		StringBuilder pagination = new StringBuilder();
		// 1. 한 화면에 보여줄 pagination의 수.
		int pageNumber = 3;
	 ...
	 * @return
	 */
	public UserReviewDomain searchOneReview(String reviewId) {
		UserReviewDomain rDomain = null;

		try {
			rDomain = rm.selectReviewDetailUser(reviewId);
		} catch (SQLException e) {
			e.printStackTrace();
		} // end try ~ catch
		System.out.println("공지사항 도메인 서비스: " + rDomain);

		return rDomain;

	} // searchOneReview

	public boolean modifyReview(UserReviewDTO rDTO) {
		boolean flag = false;

		try {
			flag = rm.updateReview(rDTO) == 1;
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public boolean removeReview(UserReviewDTO rDTO) {
		boolean flag = false;

		try {
			flag = rm.deleteReview(rDTO) == 1;
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public UserReviewDomain searchMyReview(UserReviewDTO rDTO) {
		...
		try {
			...
			if (myReview != null) {
				myReview.setName(cryptoUtil.decryptSafe(myReview.getName()));
			}
	...
	// 리뷰리스트 암호화된 이름 복호화
	private void decryptReviewAuthorInfo(List<UserReviewDomain> reviewList) {
		if (reviewList == null || reviewList.isEmpty()) {
			return;
		}
		for (UserReviewDomain review : reviewList) {
			review.setName(cryptoUtil.decryptSafe(review.getName()));
		}
	}
