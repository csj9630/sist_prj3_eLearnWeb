...
public class VideoDTO {
	...
	public void recalculate() {
		...
		if (this.videoLength == null || this.progTime == null || this.videoLength <= 0) {
			this.progress = 0;
			this.state = 0;
			return;
		}
		...
		// 1. 진행률 계산 (소수점 버림)
		this.progress = (int) ((long) this.progTime * 100 / this.videoLength);
		...
		// 2. 상태(state) 결정
		if (this.progress >= 100 || this.progTime >= this.videoLength) {
			this.state = 2; // 완료
		} else if (this.progress > 0 || this.progTime > 0) {
			this.state = 1; // 시청 중
		} else {
			this.state = 0; // 시작 전
		} // else
