...
public class InstructorReviewService {
	...
	@Autowired
	private InstructorReviewMapper rm;
	 ...
	 * @return
	 */
	public int totalCnt(InstructorReviewRangeDTO rangeDTO) {
		int totalCnt = 0;
	 ...
	 * @return
	 */
	public List<InstructorReviewDomain> searchReviewList(InstructorReviewRangeDTO rangeDTO) {
		List<InstructorReviewDomain> list = null;
	 ...
	 * @param list
	 */
	public void contentSubStr(List<InstructorReviewDomain> reviewList) {
		String content = "";
		for (InstructorReviewDomain rDomain : reviewList) {
	...
	public String pagination(InstructorReviewRangeDTO rangeDTO) {
	 ...
	 * @return
	 */
	public InstructorReviewDomain searchOneReview(String reviewId) {
		InstructorReviewDomain rDomain = null;

		try {
			rDomain = rm.selectReviewDetailInstructor(reviewId);
		} catch (SQLException e) {
			e.printStackTrace();
		} // end try ~ catch
	...
	// 리뷰리스트 암호화된 이름 복호화
	private void decryptReviewAuthorInfo(List<InstructorReviewDomain> reviewList) {
		if (reviewList == null || reviewList.isEmpty()) {
			return;
		}
		for (InstructorReviewDomain review : reviewList) {
			review.setUser_name(cryptoUtil.decryptSafe(review.getUser_name()));
		}
	}
